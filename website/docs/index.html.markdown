...
resource "vcfa_network_routed" "net" {
  # ...
}
```
...
resource "vcfa_network_routed" "net1" {
  org = "Org1"
  vdc = "Org1VDC"
...
resource "vcfa_network_routed" "net2" {
  org = "Org2"
  vdc = "Org2VDC"
...
provider "vcfa" {
  ...
  auth_type            = "integrated"
  sysorg               = "System"
  org                  = var.vcfa_org # Default for resources
  vdc                  = var.vcfa_vdc # Default for resources
  url                  = var.vcfa_url
  max_retry_timeout    = var.vcfa_max_retry_timeout
  allow_unverified_ssl = var.vcfa_allow_unverified_ssl
}
...
```json
{
  ...
  "refresh_token": "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
  "updated_by": "terraform-provider-vcfa/v3.9.0 (darwin/arm64; isProvider:true)",
...
and activating a VCFA Service Account can be found in the 
...
token may not work in recent VCFA versions.
...
provider "vcfa" {
  ...
  sysorg    = "my-org"
  auth_type = "saml_adfs"
  # If `saml_adfs_rpt_id` is not specified - VCFA SAML Entity ID will be used automatically
  saml_adfs_rpt_id = "my-custom-rpt-id"
  # saml_adfs_cookie might looking up ADFS server in VCFA with additional cookie.
...
* `user` - (Required) This is the username for Cloud Director API operations. Can also be specified
  with the `VCFA_USER` environment variable. *v2.0+* `user` may be "administrator" (set `org` or
...
results, as the metadata will be stored in state but nothing will be done in VCFA. Using `ignore_metadata_changes` with matching metadata entries
...
* `resource_type` - (Optional) Specifies the resource type which metadata needs to be ignored. If set, the resource type must be one of:
  *"vcfa_catalog"*, *"vcfa_catalog_item"*, *"vcfa_catalog_media"*, *"vcfa_catalog_vapp_template"*, *"vcfa_independent_disk"*, *"vcfa_network_direct"*,
  *"vcfa_network_isolated"*, *"vcfa_network_isolated_v2"*, *"vcfa_network_routed"*, *"vcfa_network_routed_v2"*, *"vcfa_org"*, *"vcfa_org_vdc"*, *"vcfa_provider_vdc"*,
  *"vcfa_rde" (v3.11+)*, *"vcfa_storage_profile"*, *"vcfa_vapp"*, *"vcfa_vapp_vm"* or *"vcfa_vm"*, which are the resources compatible with `metadata_entry`.
* `resource_name`- (Optional) Specifies the name of the entity in VCFA which metadata needs to be ignored. This attribute can be used with
   any kind of `resource_type`, except for *vcfa_storage_profile* which **cannot be filtered by name**.
...
* `conflict_action` - (Optional) Defines what to do if a conflict exists between a `metadata_entry` that is managed
  by Terraform, and it matches the criteria defined in the `ignore_metadata_changes` block, as the metadata will be stored in state but nothing will be done in VCFA.
...
provider "vcfa" {
  ...
  # Filters all metadata with key "Environment" or "environment" in all VCFA objects with any name.
  ignore_metadata_changes {
    key_regex = "^[Ee]nvironment$"
  }

  # Filters all metadata with key "NiceMetadataKey" in all VCFA objects named "SpecificName".
  ignore_metadata_changes {
...
Note that this argument **does not affect metadata of the [data source filters](/providers/vmware/vcfa/latest/docs/guides/data_source_filters)**.
