...
func getMapOfTests(vcfaVersion, providerUrl string) map[string]partitionInfo {
	...
	retrievedTestsFileName := getTestFileName("retrieved", vcfaVersion)
...
func getTestFileName(namePart, vcfaVersion string) string {
	prefix := os.Getenv("BUILD_NUMBER")
	if prefix == "" {
		prefix = "LOCAL"
	}
	return fmt.Sprintf("%s-tests-%s-in-node-%d-%s.txt", prefix, namePart, partitionNode, vcfaVersion)
}
...
func handlePartitioning(vcfaVersion, providerUrl string, t *testing.T) {
	...
	if numberOfPartitions == 1 {
		fmt.Printf("number of partitions (-vcfa-partitions) must be greater than 1\n")
		os.Exit(1)
	}

	// When partitioning is enabled, we must have a node identified for the current run
	if partitionNode == 0 {
		fmt.Printf("number of partitions (-vcfa-partitions) was set, but not the partition node (-vcfa-partition-node)\n")
		os.Exit(1)
	}
	...
	if len(mapOfTests) == 0 {
		...
		mapOfTests = getMapOfTests(vcfaVersion, providerUrl)
	}
	if len(mapOfTests) == 0 {
	...
	if partInfo.node == partitionNode {
		fileName := getTestFileName("processed", vcfaVersion)
		if len(listOfProcessedTests) == 0 {
			...
			writeProcessedTests(fileName, fmt.Sprintf("# VCFA    : %s", strings.Replace(providerUrl, "/api", "", 1)))
			writeProcessedTests(fileName, fmt.Sprintf("# version : %s", vcfaVersion))
		}
