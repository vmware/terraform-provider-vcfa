...
if [[ ! -f CHANGELOG.md ]]; then
  ...
  echo "Please run this from the root of the terraform-provider-vcfa repository"
  exit 1
fi
...
REPO_URL="https:\/\/github.com\/vmware\/terraform-provider-vcfa\/pull"

$SED "s/GH-([0-9]+)/\[#\1\]\($REPO_URL\/\1\)/g" -e 's/\[\[#(.+)([0-9])\)]$/(\[#\1\2))/g' CHANGELOG.md
if [ "$?" != "0" ] ; then exit 1 ; fi
rm CHANGELOG.md.bak
