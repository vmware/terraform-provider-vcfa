...
function terraform_binary_path {
    ...
    then
        target_dir=.terraform.d/plugins/registry.terraform.io/vmware/vcfa/$bare_version/$arch
    fi
...
function get_help {
    ...
    echo "  i | test-env-init      Prepares the environment in a new VCFA"
    echo "  b | test-env-apply     Builds the environment in a new VCFA"
    ...
    echo ""
    echo "If no options are given, it runs all the vcfa*.tf tests in test-artifacts"
    echo ""
    echo "Examples"
    echo "test-binary.sh tags 'org contentlibrary' clear pause"
    echo "test-binary.sh t 'org contentlibrary' c p"
    echo "test-binary.sh tags ldap dry"
    echo "test-binary.sh tags ldap upgrade v1.0.0 v1.1.0"
    echo ""
...
do
  ...
  case $opt in
    ...
    u|upgrade)
        ...
        if [ -z "$to_version" ]
        then
            echo "option 'upgrade' requires two arguments (such as 'v1.0.0' 'v1.1.0')"
            exit 1
        fi
        ;;
    n|names)
...
[ -z  "$test_names" ] && test_names='vcfa.*.tf'
